...
class JTransliteration
{
	 ...
	 *
	 *  @param $string 		The string to be transliterated
	 *
	 *  @return   Returns the transliterated string
	*/

	public static function transliterate($string)
	{
		return JTransliteration::_transliteration_process($string);
	}
	 ...
	 *   Transliterated text.
	 */
	private static function _transliteration_process($string, $unknown = '?', $source_langcode = null)
	{
		...
		foreach ($matches[0] as $str)
		{
			...
			for ($i = -1; --$len;)
			{
				...
				if ($remaining = $tail_bytes[$c])
				{
					...
					$result .= JTransliteration::_transliteration_replace($ord, $unknown, $source_langcode);
					$head = '';
				}
				elseif ($c < "\x80")
				{
...
//			$langcode = $language->language;
		...
		{
			$file = JTransliteration::_get_path() . '/data/' . sprintf('x%02x', $bank) . '.php';
			if (file_exists($file))
			{
				include $file;
	...
	/*
	 * static implemantation of drupal_get_path function
	 * needed for porting of transliteration drupal module
	 */
	private static function _get_path()
	{
